...
Joins[inner/self join,outer join(left,right,full)]
...
Select * from (Select * from Employee ORDER BY Rownum DESC) Where Rownum <= n; //displays the last n number of records
...
select empname,annualsalary/12 from emp_tbl
...
select * from emp_tbl where to_char(joiningdate,'YYYY') = '2018' //Joining date using typecasting
...
Once the foreign key references to primary key , you cannot delete that Primary key COLUMN of the child table.
...
4.This adds a Clustered INDEX Automatically                    This doesnot add any Clustered INDEX Automatically
-------------------------------------------------------------------------------------------------------------
...
Ans:
select A.Name as EmployeeName,B.Name as ManagerName from Emp as A,Emp as B where A.id=B.id;
select A.Name as EmployeeName,B.Name as ManagerName from Emp as A inner join Emp as B On A.id=B.id;
*************************************************************************************
...
insert into emptbl values(20,'Global','Mailasandra') //If we want to insert a new record completely
...
Non Co-related subquery: inner query not depends on outer query
